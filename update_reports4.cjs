...
let target1 = `    const closeModal = () => { modal.style.display = 'none'; };\r\n    document.getElementById('close-modal-fin-window')?.addEventListener('click', closeModal);`;
let target2 = `    const closeModal = () => { modal.style.display = 'none'; };\n    document.getElementById('close-modal-fin-window')?.addEventListener('click', closeModal);`;

if (content.includes(target1)) {
    content = content.replace(target1, scriptToInsert);
    fs.writeFileSync(path, content, 'utf8');
    console.log('Update complete target1');
} else if (content.includes(target2)) {
    content = content.replace(target2, scriptToInsert);
    fs.writeFileSync(path, content, 'utf8');
    console.log('Update complete target2');
} else {
