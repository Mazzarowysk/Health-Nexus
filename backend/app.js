  ...
  try {
    ...
    if (pCount === 0) {
      const demoPatients = [
        ...
        { id: 'PAT-DEMO-02', fullName: 'Beatriz Fagundes Silveira', cpf: '111.222.333-02', birthDate: '1992-08-25', phone: '(11) 3111-0002', cellphone: '(11) 91111-0002', address: 'Rua Augusta, 450 - Consolação', city: 'São Paulo - SP', billingValue: 'Bradesco Saúde Especial' },
        ...
        { id: 'PAT-DEMO-04', fullName: 'Caio Roberto Vasconcelos', cpf: '111.222.333-04', birthDate: '1990-02-14', phone: '(11) 3111-0004', cellphone: '(11) 91111-0004', address: 'Av. Faria Lima, 2100 - Itaim Bibi', city: 'São Paulo - SP', billingValue: 'Particular' },
        { id: 'PAT-DEMO-05', fullName: 'Camila Teixeira Medeiros', cpf: '111.222.333-05', birthDate: '1997-06-30', phone: '(11) 3111-0005', cellphone: '(11) 91111-0005', address: 'Rua Oscar Freire, 320 - Jardins', city: 'São Paulo - SP', billingValue: 'Amil 500' },
        ...
        { id: 'PAT-DEMO-09', fullName: 'Fernanda Souza Leão', cpf: '111.222.333-09', birthDate: '1994-05-16', phone: '(11) 3111-0009', cellphone: '(11) 91111-0009', address: 'Rua Teodoro Sampaio, 1100 - Pinheiros', city: 'São Paulo - SP', billingValue: 'Unimed Omega' },
        ...
        { id: 'PAT-DEMO-15', fullName: 'Mariana Peixoto Guimarães', cpf: '111.222.333-15', birthDate: '1995-02-28', phone: '(11) 3111-0015', cellphone: '(11) 91111-0015', address: 'Rua Pamplona, 700 - Jardim Paulista', city: 'São Paulo - SP', billingValue: 'Porto Seguro Prata' },
        ...
        { id: 'PAT-DEMO-21', fullName: 'Sophia Alcantara Montez', cpf: '111.222.333-21', birthDate: '1997-10-14', phone: '(11) 3111-0021', cellphone: '(11) 91111-0021', address: 'Rua Frei Caneca, 560 - Consolação', city: 'São Paulo - SP', billingValue: 'Porto Seguro Ouro' },
        ...
        { id: 'PAT-DEMO-25', fullName: 'Xavier Camargo Filho', cpf: '111.222.333-25', birthDate: '1994-09-24', phone: '(11) 3111-0025', cellphone: '(11) 91111-0025', address: 'Rua Pedroso Alvarenga, 900 - Itaim Bibi', city: 'São Paulo - SP', billingValue: 'SulAmérica Especial' },
      ...
      const sampleAppointments = [
        ...
        { patientId: 'PAT-DEMO-04', patientName: 'Caio Roberto Vasconcelos', doctorName: 'Dr. Carlos Oliveira', specialty: 'Clínica Geral', time: '09:15', status: 'Em Atendimento', notes: 'Check-up anual com solicitação de exames bioquímicos completos.' },
        { patientId: 'PAT-DEMO-05', patientName: 'Camila Teixeira Medeiros', doctorName: 'Dra. Ana Costa', specialty: 'Ortopedia', time: '10:00', status: 'Agendado', notes: 'Avaliação de entorse no tornozelo esquerdo. Solicitação de RX.' },
...
const initCloudDb = async () => {
  ...
  const tasks = async () => {
    ...
    await cloud.execute(SQL_USERS);
    for (const col of ["status TEXT DEFAULT 'Ativo'", 'master_approved INTEGER DEFAULT 1', 'master_key_requested INTEGER DEFAULT 0']) {
      try { await cloud.execute(`ALTER TABLE users ADD COLUMN ${col}`); } catch (e) {}
    }
    try { await cloud.execute('ALTER TABLE users RENAME COLUMN email TO username'); } catch (e) {}
    await cloud.execute(SQL_PATIENTS);
...
// --- ENDPOINT: Varredura de Status da Nuvem (usado pós-login no Vercel e local) ---
app.get('/api/sync/cloud-status', authenticateToken, async (req, res) => {
  try {
    const cloudDb = getCloudDb();
    if (!cloudDb) {
      return res.json({ cloudConfigured: false, hasData: false });
    }

    const tables = ['patients', 'appointments', 'encounters', 'doctors', 'beds', 'prescriptions', 'pharmacy_items'];
    const counts = {};
    let totalRecords = 0;
    ...
    for (const table of tables) {
      try {
        const r = await cloudDb.execute(`SELECT COUNT(*) as c FROM ${table}`);
    ...
    res.status(500).json({ cloudConfigured: false, hasData: false, error: err.message });
  }
});
...
// --- FINANCIAL PDF EXPORT ENDPOINT ---
app.post('/api/financial/export-pdf', async (req, res) => {
  try {
    const { title, columns, rows, financialSummary } = req.body || {};
    if (!columns || !rows) {
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="footer">
          Health Nexus © 2026 — Sistema Integrado de Saúde Hospitalar • Documento gerado via PDF Server-Side
        </div>
      </body>
      </html>
    `;
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${title.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().slice(0,10)}.pdf"`);
    res.send(pdfBuffer);
